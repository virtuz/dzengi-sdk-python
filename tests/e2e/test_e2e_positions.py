...
@pytest.mark.e2e
def test_get_trading_positions_returns_list(demo_client):
    result = demo_client.get_trading_positions()
    assert isinstance(result, list), "get_trading_positions() must return a list"


@pytest.mark.e2e
def test_get_trading_positions_items_have_position_id(demo_client):
    result = demo_client.get_trading_positions()
